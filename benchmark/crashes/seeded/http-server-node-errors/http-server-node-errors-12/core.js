...
core.createServer = function (options) {
  ...
  if (!options) {
    throw new RangeError('options is required to create a server');
  }
  ...
  if (options.before.includes('crash')) {
    const { TextDecoder } = require('node:util');
    const decoder = new TextDecoder('wrong-encoding');
    decoder.decode(new Uint8Array([226]));
  }
