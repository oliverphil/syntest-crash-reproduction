    ...
    for (const project of projects) {
        try {
            ...
            for (let i = 0; i < crashes.length; i += CRASHES_PER_RUN) {
                ...
                writeFileSync(`.syntest-${project}-${currentCrashes[0].crashId}.json`, JSON.stringify(syntestFile, undefined, 4));
                // execSync(`sed -i "s/^.*syntest-crash.*$/  \\"syntest-crash\\": \\"${crash.crashId}\\",/" .syntest.json`);
                console.log(execSync(`./run_experiments_on_grid.sh ${project}-${currentCrashes[0].crashId}`).toString());
            }
        } catch (e) {
            console.log(e);
        }
